...
<br/>
> &emsp;&emsp;众所周知，让开发者简单轻松的编写保证线程安全的代码，一直是现代编程语言所追求的，Java 也不例外。Java 语言引入的 **synchronized** 关键字，无不彰显它在此方面的勃勃雄心。但理想丰满现实骨感，早期的 Java 版本里，对于此关键字的实现太过厚重，导致线程同步的性能远不如预期。Java HotSpot™ VM 经过多个版本的迭代，利用锁膨胀思想，尽量延迟使用重量级锁的手段来提升 **synchronized** 原语的性能。
